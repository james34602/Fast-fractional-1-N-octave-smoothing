...
void makimaUpdate(ierper *intp, double *y, int left_endpoint_derivative, int right_endpoint_derivative)
{
	int n = intp->n;
	...
	double* pc2 = (double*)(((char*)intp) + sizeof(ierper) + sizeof(double) * n * 2 + sizeof(double) * 3);
	double* pc3 = (double*)(((char*)intp) + sizeof(ierper) + sizeof(double) * n * 2 + sizeof(double) * 3 + sizeof(double) * (n - 4) * 4);
	double m2 = (y[3] - y[2]) * pc1[0];
	double m1 = (y[2] - y[1]) * pc1[1];
	double m0 = (y[1] - y[0]) * pc1[2];
	...
	for (int i = 2; i < n - 2; ++i)
	{
		double mim2 = (y[i - 1] - y[i - 2]) * pc2[(i - 2) + minus4 * 0];
		double mim1 = (y[i] - y[i - 1]) * pc2[(i - 2) + minus4 * 1];
		double mi = (y[i + 1] - y[i]) * pc2[(i - 2) + minus4 * 2];
		double mip1 = (y[i + 2] - y[i + 1]) * pc2[(i - 2) + minus4 * 3];
	...
	// Quadratic extrapolation at the other end:
	double mnm4 = (y[n - 3] - y[n - 4]) * pc3[0];
	double mnm3 = (y[n - 2] - y[n - 3]) * pc3[1];
	double mnm2 = (y[n - 1] - y[n - 2]) * pc3[2];
